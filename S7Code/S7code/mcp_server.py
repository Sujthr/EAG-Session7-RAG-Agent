...
Twelve tools, stdio transport:
    ...
    index_document, index_folder, search_knowledge
...
index_folder:      Index every .md (or specified extension) file in a sandbox
                   folder in one call. Returns total chunks indexed and a per-file
                   breakdown. Use instead of calling index_document in a loop.
...
@mcp.tool()
def index_folder(folder_path: str = "papers", extension: str = ".md", chunk_size: int = 400, overlap: int = 80) -> dict:
    """Index every file matching `extension` inside a sandbox folder in one shot. Returns total_chunks_indexed and a per-file breakdown. Use this to bulk-index a corpus instead of calling index_document in a loop. Example: index_folder("papers") indexes all .md files under sandbox/papers/."""
    folder = _safe(folder_path)
    if not folder.is_dir():
        return {"error": f"'{folder_path}' is not a directory inside the sandbox"}
    files = sorted(folder.rglob(f"*{extension}"))
    if not files:
        return {"folder": folder_path, "files_found": 0, "total_chunks_indexed": 0}
    run_id = f"index-folder-{datetime.now().strftime('%Y%m%d%H%M%S')}"
    total_chunks = 0
    ...
    for fp in files:
        ...
        try:
            ...
            if not text.strip():
                file_results.append({"file": rel, "chunks": 0, "status": "empty"})
                continue
            chunks = _chunk_text(text, size=chunk_size, overlap=overlap)
            ...
            total_chunks += len(chunks)
            file_results.append({"file": rel, "chunks": len(chunks), "status": "ok"})
        except Exception as exc:
            file_results.append({"file": rel, "chunks": 0, "status": f"error: {exc}"})
    return {
        "folder": folder_path,
        "extension": extension,

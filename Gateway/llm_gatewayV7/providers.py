...
class OpenAIProvider(OpenAICompatProvider):
    """Native OpenAI API — paid, last-resort fallback.

    Default model: gpt-4.1-mini
      - 1M context window, ~$0.40/1M input tokens
      - Outperforms gpt-4o-mini on most benchmarks at similar cost
      - Full tool-calling and JSON-schema structured output support
...
# ── Gemini model cascade order ────────────────────────────────────────────────
# Cheapest / fastest first; most capable last as final fallback.
# Override with GEMINI_MODEL_ORDER env var (comma-separated).
GEMINI_DEFAULT_MODEL_ORDER = [
...
class GeminiCascadeProvider(GeminiProvider):
    """Full Gemini cascade: model order × key rotation.

    Outer loop: model cascade (cheapest → most capable)
        gemini-2.5-flash-lite-preview-06-17 → gemini-2.5-flash-lite →
        gemini-2.0-flash-lite → gemini-2.5-flash

    Inner loop: key rotation across all 5 API keys on 429 / 403.

    Decision rules:
      - 429 / 403  → quota / auth error: rotate to next key (same model).
      - 404 / 400  → model unavailable:  skip remaining keys, advance model.
      - other      → non-retriable: raise immediately.

    Override model order with GEMINI_MODEL_ORDER env var (comma-separated).
    Set GEMINI_API_KEY_1 … GEMINI_API_KEY_5 for up to 5 keys.
    """

    def __init__(self, api_keys: list[str], model_order: list[str], cache_store):
        # Initialise parent with the first (cheapest) model and first key.
        super().__init__(api_keys[0], model_order[0], cache_store)
        self._keys = api_keys
        self._model_order = model_order
        # Remember last-successful position to start there on the next call
        # (avoids always starting from the slowest / most expensive fallback).
        self._last_model_idx = 0
        self._last_key_idx = 0

    async def chat(self, messages, *, max_tokens=2048, temperature=0.7, model=None,
                   tools=None, tool_choice=None, reasoning=None, response_format=None,
                   system_blocks=None, cache_system=False):
        # Explicit model override from caller: bypass cascade entirely.
        if model:
            self.model = model
            self.api_key = self._keys[self._last_key_idx]
        ...
        for m_idx, current_model in enumerate(self._model_order):
            self.model = current_model
            for k_idx, current_key in enumerate(self._keys):
                self.api_key = current_key
                try:
                    result = await super().chat(
                        messages, max_tokens=max_tokens, temperature=temperature,
                        model=None, tools=tools, tool_choice=tool_choice,
                        reasoning=reasoning, response_format=response_format,
                        system_blocks=system_blocks, cache_system=cache_system,
                    )
                    # Success — remember position for the next call.
                    self._last_model_idx = m_idx
                    self._last_key_idx = k_idx
                    return result
                except ProviderError as e:
                    last_err = e
                    if e.status in (429, 403):
                        # Quota / auth: try next key for the same model.
                        if k_idx < len(self._keys) - 1:
                            print(f"[gemini_cascade] {current_model} key {k_idx + 1}"
                                  f" -> {e.status}, rotating key")
                            continue
                        # All keys exhausted for this model -> advance model.
                        print(f"[gemini_cascade] {current_model} all keys exhausted"
                              f" -> advancing to next model")
                        break
                    elif e.status in (404, 400):
                        # Model unavailable / bad request: skip to next model.
                        print(f"[gemini_cascade] {current_model} -> {e.status}"
                              f" (model issue), skipping to next model")
                        break
                    else:
                        # Unexpected non-retriable error — surface immediately.
                        raise

        raise ProviderError(
            f"gemini cascade exhausted all {len(self._model_order)} models"
            f" x {len(self._keys)} keys. last_error={last_err}",
    ...
    def active_model(self) -> str:
        return self._model_order[self._last_model_idx]

    @property
    def active_key_slot(self) -> int:
        return self._last_key_idx + 1

    @property
    def model_count(self) -> int:
        return len(self._model_order)
...
def _collect_gemini_keys() -> list[str]:
    """Read GEMINI_API_KEY_1 … GEMINI_API_KEY_5 in order.
...
def _collect_gemini_model_order() -> list[str]:
    """Read GEMINI_MODEL_ORDER env var (comma-separated) or use default cascade."""
    env = os.getenv("GEMINI_MODEL_ORDER", "").strip()
    if env:
        order = [m.strip() for m in env.split(",") if m.strip()]
        if order:
            return order
    return list(GEMINI_DEFAULT_MODEL_ORDER)
...
def build_providers(cache_store):
    ...
    V7 adds GeminiCascadeProvider: tries models cheapest-first, rotates keys on
    quota errors. Configure with GEMINI_MODEL_ORDER (comma-separated) and
    GEMINI_API_KEY_1 … GEMINI_API_KEY_5.
    """
    out = {}
    gemini_keys = _collect_gemini_keys()
    if gemini_keys:
        model_order = _collect_gemini_model_order()
        out["gemini"] = GeminiCascadeProvider(gemini_keys, model_order, cache_store)
        print(
            f"[providers] Gemini cascade active: {len(model_order)} models"
            f" x {len(gemini_keys)} keys"
        )
        print(f"[providers] Gemini model order: {' -> '.join(model_order)}")
    ...
    if k := os.getenv("OPENAI_API_KEY"):
        if k.startswith("sk-") or k.startswith("sk-proj-"):
            ...
            print(f"[providers] OpenAI fallback active: {os.getenv('OPENAI_MODEL', 'gpt-4.1-mini')}")
        else:
            print("[providers] OpenAI key looks like a placeholder — skipping")
    return out

...
def _should_retry(exc: Exception) -> bool:
    ...
    # Only retry connection-level errors — NOT RemoteProtocolError/ReadTimeout,
    # which indicate a persistent gateway/provider issue. Avoid blocking the
    # asyncio event loop for 30+ seconds on every failed call.
...
class LLM:
    ...
    def chat(self, prompt: str = None, *,
        ...
        for attempt, delay in enumerate([0] + list(_RETRY_DELAYS)):
            if delay:
                code = getattr(getattr(last_exc, "response", None), "status_code",
                               type(last_exc).__name__)
                print(f"[llm-client] retry {attempt}/{len(_RETRY_DELAYS)} after {code}: "
                      f"{str(last_exc)[:80]}, waiting {delay}s...")
                time.sleep(delay)
            try:
    ...
    def embed(self, text: str,
        ...
        for attempt, delay in enumerate([0] + list(_RETRY_DELAYS)):
            if delay:
                code = getattr(getattr(last_exc, "response", None), "status_code",
                               type(last_exc).__name__)
                print(f"[llm-client] embed retry {attempt}/{len(_RETRY_DELAYS)} after {code}: "
                      f"{str(last_exc)[:80]}, waiting {delay}s...")
                time.sleep(delay)
            try:

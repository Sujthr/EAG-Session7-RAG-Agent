...
class GeminiKeyRingEmbedder(GeminiEmbedder):
    """GeminiEmbedder with up to 5 API keys that rotate on 429 rate-limit errors.

    Reads GEMINI_API_KEY_1 … GEMINI_API_KEY_5 in order, with GEMINI_API_KEY
    as the legacy single-key fallback. On a 429 the next key is selected and
    the call is retried immediately, so the embedding call succeeds as long as
    at least one key has quota.
    """

    def __init__(self, api_keys: list[str], model: str, output_dim: int = EMBED_DIM,
                 rpm: int = 5, cooldown: float = 2.0):
        super().__init__(api_keys[0], model, output_dim, rpm, cooldown)
        self._keys = api_keys
        self._key_idx = 0
    ...
    async def embed(self, text: str, task_type: TaskType) -> dict:
        last_err: EmbedderError | None = None
        for attempt in range(len(self._keys)):
            try:
                # Temporarily swap the key for this attempt.
                saved_key = self.api_key
                self.api_key = self._keys[self._key_idx]
                result = await super().embed(text, task_type)
                return result
            except EmbedderError as e:
                last_err = e
                if e.status == 429 and attempt < len(self._keys) - 1:
                    new_key = self._advance_key()
                    print(f"[gemini_embed_ring] key rotated to slot {self._key_idx + 1} after 429")
                    continue
                raise
...
def _collect_gemini_embed_keys() -> list[str]:
    """Collect GEMINI_API_KEY_1 … GEMINI_API_KEY_5, fallback to GEMINI_API_KEY."""
    keys: list[str] = []
    for i in range(1, 6):
...
def build_embedders() -> tuple[list[EmbeddingProvider], list[str]]:
    ...
    V7 adds Gemini key-ring support: set GEMINI_API_KEY_1 … GEMINI_API_KEY_5
    for automatic key rotation when an embedding call hits a 429.
    """
    ...
    if fallback_provider == "gemini":
        gemini_keys = _collect_gemini_embed_keys()
        if gemini_keys:
            if len(gemini_keys) == 1:
                registry["gemini"] = GeminiEmbedder(gemini_keys[0], fallback_model)
            else:
                registry["gemini"] = GeminiKeyRingEmbedder(gemini_keys, fallback_model)
                print(f"[embedders] Gemini embed key-ring active: {len(gemini_keys)} keys")

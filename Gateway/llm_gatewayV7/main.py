...
# Tier -> worker failover order.
# Ollama is PRIMARY per spec (offline-first). Gemini/Groq/NVIDIA are fallbacks.
TIER_TO_ORDER = {
    "TINY":  ["ollama", "groq", "gemini", "nvidia", "github", "openrouter", "cerebras", "openai"],
    "LARGE": ["ollama", "gemini", "groq", "nvidia", "github", "openrouter", "cerebras", "openai"],
}
...
async def chat(req: ChatRequest):
    ...
    for _ in range(len(candidates) + 1):
        ...
        try:
            ...
            # Remove this provider and continue to the next candidate.
            # Only stop the whole cascade if there are no more candidates left
            # AND this error was non-retryable (e.g. Gemini exhausted all keys).
            candidates = [c for c in candidates if c != name]
            if not getattr(e, "retryable", True) and not candidates:
                raise HTTPException(502, f"all providers exhausted. last={name}: {e}")
            continue
